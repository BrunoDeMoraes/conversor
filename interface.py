...
class Conversor:
    opções = [
        'Selecione uma opção','Converter pdf em jpg', 'Mesclar arquivos'
    ]
    ...
    def __init__(self, tela):
        ...
        self.titulo = Label(
            self.frame_mestre, text='Conversor', pady=0, padx=200, bg='red',
            fg='white', bd=2, relief=SUNKEN, font=('Helvetica', 12, 'bold')
        )

        self.botão_de_seleção_de_aquivos = Button(
            self.frame_de_botões, text='Incluir arquivos na lista',
            command=self.seleciona_arquivos, padx=20, pady=0, bg='white',
            fg='red', font=('Helvetica', 9, 'bold'), bd=1
        )

        self.botão_excluir_arquivos_da_lista = Button(
            self.frame_de_botões, text='Retirar arquivos da lista',
            command=self.exclui_arquivos_da_lista, padx=20, pady=0,
            bg='white', fg='red', font=('Helvetica', 9, 'bold'), bd=1
        )

        self.listbox_de_arquivos = Listbox(
            self.frame_mestre, width=70, selectmode=EXTENDED
        )

        self.variavel_de_opções = StringVar()
        self.variavel_de_opções.set("Selecione uma opção")
        self.validacao = OptionMenu(
            self.frame_mestre, self.variavel_de_opções, *Conversor.opções
        )

        self.roda_pe = Label(
            self.frame_mestre, text="SRSSU/DA/GEOF   ", pady=0, padx=0,
            bg='red', fg='white', font=('Helvetica', 8, 'italic'), anchor=E
        )

        self.botão_converter = Button(
            self.frame_mestre, text='Executar', padx=0, pady=0, bg='red',
            fg='white', font=('Helvetica', 9, 'bold'), bd=1,
            command=self.selecionador_de_opções
        )

        self.titulo.grid(row=0, column=1, pady=10, padx=0, sticky=W+E)
        self.frame_de_botões.grid(row=1, column=1, sticky=W+E)
        self.botão_de_seleção_de_aquivos.grid(
            row=1, column=1, padx=35, pady=5
        )
        self.botão_excluir_arquivos_da_lista.grid(
    ...
    def exclui_arquivos_da_lista(self):
        if not self.listbox_de_arquivos.curselection():
            messagebox.showerror(
                'Não dá pra apagar o que não existe!',
                'Não há arquivos listados ou selecionados.'
            )
        else:
            for arquivo in self.listbox_de_arquivos.curselection():
                messagebox.showinfo(
                    'Foi-se!',
                    (
                        f'O arquivo ({self.listbox_de_arquivos.get(arquivo)})'
                        f' foi excluído da lista para conversão.'
                    )
                )
                del self.lista_de_arquivos[
                    self.listbox_de_arquivos.get(arquivo)
                ]
            self.atualiza_listbox()
        ...
        if not self.lista_de_arquivos.keys():
            ...
            for arquivo in self.lista_de_arquivos:
                ...
                for página in páginas:
                    if contador < 9:
                        páginas[contador].save(
                            (f"{caminho_para_salvar_arquivos}/{arquivo_base} "
                             f"- {0}{contador + 1}.jpg"),
                            "JPEG"
                        )
                        contador += 1
            ...
            for arquivo_convertido in número_de_páginas_dos_arquivos:
                páginas_por_arquivo += (
                    f'{arquivo_convertido}; Número de imagens: '
                    f'{número_de_páginas_dos_arquivos[arquivo_convertido]}.\n'
                )
            messagebox.showinfo(
                'Rolou tranquilo!', f'Imagens criadas com sucesso!\n\n'
                                    f'{páginas_por_arquivo}'
            )
    ...
    def mesclar_arquivos_pdf(self):
        if not self.lista_de_arquivos.keys():
            ...
            caminho_para_salvar_arquivos = filedialog.askdirectory()
            if os.path.exists(
                    f'{caminho_para_salvar_arquivos}/Arquivos_mesclados.pdf'
            ):
                messagebox.showwarning(
                    'Encreca em dobro!',
                    (
                        'Já existe um arquivo com o nome de "Arquivos_mesclad'
                        'os" na pasta que você está selecionou para criação d'
                        'o novo arquivo.\n\nRemova, apague ou renomeie o arqu'
                        'ivo existente antes de prosseguir com a ação.'
                    )
                )
            else:
                with open(
                        (
                                f'{caminho_para_salvar_arquivos}/Arquivos_mes'
                                f'clados.pdf'
                        ), 'wb'
                ) as arquivo_final:
